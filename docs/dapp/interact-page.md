---
title: dApp Interact Page
description: How merchants and advanced users call smart contract functions directly through the ABI interface in the Subscrypts dApp.
tags:
 - dapp
 - interact
 - abi
...
| ---------------------------- | --------------------------------------------- | ------------- |
| `subscriptionRecurringCHG()` | Toggles auto-renew on or off                  | Yes             |
| `getPlans()`                 | Lists all plans on-chain                      | No             |
| `subscriptionGift()`         | Creates a gifted subscription                 | Yes             |
| `getSubscriptionsByPlan()`   | Retrieves all subscriber addresses for a plan | No             |
...
For more information on how access permissions and output handling are implemented, see [Access Control & Outputs](access-control.md).

!!! info "Ecosystem Connection"
    The Interact page exposes the full **Subscrypts Smart Contract Suite** ABI. For complete function signatures and low-level details, see the [Subscrypts ABI Reference](../smart-contract/abi-reference.md). For access control details at the contract level, see [Smart Contract Access Control](../smart-contract/access-control.md).
...
* **Warning:** Read-only call returned empty or unexpected data.
...
All function names, descriptions, and parameter definitions in the Interact page are derived from the **[Subscrypts ABI Reference](../smart-contract/abi-reference.md)**. This ensures one-to-one accuracy with the deployed contracts.
...
- **[Smart Contract Functions in the dApp](smart-contract-functions.md)** -- How each dApp action maps to specific contract functions.
- **[Access Control & Outputs](access-control.md)** -- How permissions and event outputs work in the dApp.
- **[Subscrypts ABI Reference](../smart-contract/abi-reference.md)** -- Full ABI details for all contract functions.
- **[dApp Wallet Connection Guide](wallet-connection.md)** -- Wallet setup required before using the Interact page.
- **[Smart Contract Architecture](../smart-contract/architecture.md)** -- How facets and the proxy pattern work under the hood.
