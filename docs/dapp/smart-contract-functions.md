---
title: dApp Smart Contract Functions Reference
description: Reference of all smart contract functions accessible through the Subscrypts dApp, with purpose, parameters, and events.
tags:
  - dapp
  - smart-contracts
  - functions
...
!!! info "Ecosystem Connection"
    This page documents how the dApp maps to the **Smart Contract Suite**. For the full contract architecture and design, see [Smart Contract Architecture](../smart-contract/architecture.md) and [Smart Contract Core Logic](../smart-contract/core-logic.md).
...
!!! info "Access Modifiers"
    Some functions are restricted by modifiers such as `onlyOwner`, `onlyServiceAccounts`, or `onlySelfProxy`. These functions are still *publicly callable* at the EVM level, but will revert unless the caller has the required role.
...
- **[Access Control & Outputs](access-control.md)** -- How functions and events are surfaced in the dApp UI and how roles are enforced.
- **[dApp Interact Page](interact-page.md)** -- Call these functions directly through the browser.
- **[Subscrypts ABI Reference](../smart-contract/abi-reference.md)** -- Full ABI details with types, selectors, and signatures.
- **[Smart Contract Events & Integrations](../smart-contract/events-integrations.md)** -- How events propagate to the Discord Bot and external systems.
- **[Discord Bot Subscription Sync](../discord-bot/subscription-sync.md)** -- How subscription events drive Discord role management.
