---
title: dApp Wallet Connection Guide
description: How wallet connections work in the Subscrypts dApp, including supported wallets, network detection, and transaction security.
tags:
 - dapp
 - wallet
 - connection
...
!!! tip
    Ensure your wallet has the latest version and supports the Arbitrum network before connecting.
...
!!! warning
    Subscrypts transactions will not function correctly on other networks such as Ethereum Mainnet or Polygon.
...
!!! info "Ecosystem Connection"
    All wallet-signed transactions interact with the **Subscrypts Smart Contract Suite** on Arbitrum. For details on the underlying contract functions, see [Smart Contract Functions in the dApp](smart-contract-functions.md) and the [Subscrypts ABI Reference](../smart-contract/abi-reference.md).
...
Users can independently verify contract addresses against the official [Subscrypts ABI Reference](../smart-contract/abi-reference.md).

!!! info
    Broader adoption of the **Subscrypts ABI** by Web3 wallet providers is encouraged to improve transaction readability and UX. When wallets natively recognize the Subscrypts ABI, users will see contract interactions presented in clear, human-readable descriptions directly in the wallet interface -- reducing confusion and strengthening trust during transaction signing.
...
!!! info
    Many mobile browsers shipped by phone manufacturers -- such as **Apple Safari**, **Google Chrome (mobile)**, and **Samsung Internet Browser** -- do **not** natively support injected Web3 providers like MetaMask or WalletConnect.
...
- **[dApp Subscriber Guide](subscriber-guide.md)** -- Full walkthrough of the subscriber journey after connecting a wallet.
- **[dApp Merchant Guide](merchant-guide.md)** -- How merchants use the dApp after wallet connection.
- **[dApp Interact Page](interact-page.md)** -- Call smart contract functions directly through the browser.
- **[Smart Contract Suite Overview](../smart-contract/introduction.md)** -- The on-chain contracts your wallet interacts with.
- **[SDK Wallet Connectors](../sdk/wallet-connectors.md)** -- Developer reference for wallet integration via the SDK.
