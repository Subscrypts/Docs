---
title: Discord Bot Security & Trust
description: How the Subscrypts Discord Bot ensures user privacy, secure wallet linking, and MiCAR-aligned subscription access.
tags:
 - subscrypts
 - discord
 - security
...
!!! success "Tamper-Proof Subscribe Links"
...
!!! info "Blockchain Always Wins"
    Caching makes the system faster and more robust, but **the blockchain always wins** in case of conflict.

For a deeper look at this behavior, see **[Subscription Synchronization](subscription-sync.md)**.
...
* Use **private threads** and correctly configured channels (as described in **[Getting Started for Admins](admin-setup-guide.md)**) to keep wallet linking flows and admin logs scoped to the right audience.
...
!!! ecosystem "Subscrypts Ecosystem"
    The security model described here is shared across the [Smart Contract Suite](../smart-contract/introduction.md), the [Subscrypts dApp](../dapp/introduction.md), and the Discord Bot. All interfaces read from the same on-chain source of truth without custodial risk.
...
- [Architecture & Integration](architecture.md) -- How all components fit together
- [Subscription Synchronization](subscription-sync.md) -- How on-chain state becomes Discord roles
- [Best Practices & Troubleshooting](best-practices.md) -- Tips to keep your server safe and reliable
- [Admin Setup Guide](admin-setup-guide.md) -- Server configuration and permission setup
